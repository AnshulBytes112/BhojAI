...
router.get('/', 
  ...
  // Exclude reservations by default unless explicitly requested
  if (includeReservations !== 'true') {
    where['type'] = { notIn: ['RESERVATION', 'PRE_ORDER'] };
  }
...
router.post('/', 
  ...
  try {
    ...
    const isReservationOrder = orderType === 'RESERVATION' || orderType === 'PRE_ORDER';

    if (!isReservationOrder && (!items || items.length === 0)) {
      return res.status(400).json({ error: 'Order must have at least one item' });
    }

    if (!isReservationOrder && items.some((item: { quantity: number }) => !item.quantity || item.quantity <= 0)) {
    ...
    const menuItemIds = isReservationOrder ? [] : safeItems.map((i: { menuItemId: string }) => i.menuItemId);
    const menuItems = isReservationOrder
      ? []
    ...
    if (!isReservationOrder && menuItems.length !== menuItemIds.length) {
    ...
    const order = await prisma.order.create({
      data: {
        ...
        diningArea: req.body.diningArea || null,
        ...(isReservationOrder
          ? {}
          : {
              items: {
    ...
    if (!isReservationOrder) {
      await prisma.kOT.create({
        data: {
          ...
          status: 'PENDING',
          items: {
            create: safeItems.map((item: { menuItemId: string; quantity: number; notes?: string }) => ({
    ...
    await prisma.auditLog.create({
      data: {
        ...
        description: isReservationOrder
          ? `Reservation created (${orderType})`
          : `Order created with ${safeItems.length} item(s)`,
        orderId: order.id,
        userId: req.user!.id,
      },
    });

    // Create initial unpaid bill automatically so it shows in dashboard
    if (!isReservationOrder) {
      const orderForBill = await prisma.order.findUnique({
        where: { id: order.id },
        include: { items: true, restaurant: true, bill: true },
      });

      if (orderForBill && !orderForBill.bill) {
        await generateBill(orderForBill as any);
      }
    }
...
router.patch('/:id/items', async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Automatically update bill totals if bill exists, otherwise create it
    if (updated?.bill) {
      await recalculateBill(updated.bill.id);
    } else if (updated && !updated.bill && updated.items.length > 0) {
      // Create bill if it was missed during initial creation (e.g. no items before)
      await generateBill(updated as any);
    }
...
router.post('/:id/payment', async (req: AuthRequest, res: Response) => {
  try {
    ...
    let paymentAmount = typeof amount === 'number' ? amount : Number(amount);
    
    // Check for invalid or empty amount
    if (!amount && amount !== 0) {
      console.error('Payment failed: amount is missing in request body:', req.body);
      return res.status(400).json({ error: 'Payment amount is required' });
    }

    if (!Number.isFinite(paymentAmount)) {
      console.error('Payment failed: invalid amount received:', amount);
    ...
    console.log('Payment processing:', {
      billTotal: order.bill.totalAmount,
      alreadyPaid,
      remaining,
      paymentAmount,
    ...
    try {
      ...
      await prisma.auditLog.create({
        data: {
          ...
          description: `Payment of Rs.${paymentAmount} via ${method || 'CASH'}`,
          orderId: order.id,
          userId: req.user!.id,
        },
      });
