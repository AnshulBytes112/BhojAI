...
export default function MenuPage() {
  ...
  const loadMenu = async () => {
    try {
      const data = await apiRequest<MenuCategory[]>('/menu/categories');
      if (data.length) {
        setCategories(data);
        return;
      }
  ...
  const handleCreateItem = async (event: FormEvent) => {
    ...
    try {
      ...
      const localItem: MenuItem = {
        ...
        dietaryLabel: payload.dietaryLabel,
      ...
      setCategories((prev) =>
        prev.map((category) =>
          category.id === payload.categoryId ? { ...category, items: [...category.items, localItem] } : category
        )
      );
      setItemForm(EMPTY_ITEM_FORM);
      setShowItemModal(false);
      addToast({ icon: 'i', title: 'Preview item added', message: 'Saved locally because the backend was unavailable.' });
    } finally {
      setSaving(false);
    }
  };
